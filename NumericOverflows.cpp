...
{
    T result = start;
    T max = std::numeric_limits<T>::max();  // Max value for type

    // Loop through each increment and check if it will
    // go over the max value the variable can hold
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (result > (max - increment)) { // max - increment is the highest value that can be reached before an overflow occurs
            return 99; //Arbitrary number to denote overflow
        }
        result += increment;
    }
    return result;
...
{
    T result = start;
    T lowest = std::numeric_limits<T>::lowest(); // Min value for type

    // Loop through each decrement and check if it will
    // go over the lowest value the variable can hold
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (result < (lowest + decrement)) { // lowest + decrement is the lowest value that can be reached before an underflow occurs
            return 99; //Arbitrary number to denote underflow
        }
        result -= decrement;
    }
    return result;
...
void test_overflow()
{
    ...
    // The final result is displayed unless an overflow occurs then the error message will be shown
    if (+result == 99) {
...
void test_underflow()
{
    ...
    // The final result is displayed unless an underflow occurs then the error message will be shown
    if (+result == 99) {
